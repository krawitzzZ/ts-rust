...
export type Ok<T, E> = IResult<T, E> & { readonly value: T };

export type Err<T, E> = IResult<T, E> & { readonly error: E };
...
/**
 * Checks if a value is a {@link Result}, narrowing its type to `Result<unknown, unknown>`.
 *
 * This type guard determines whether the input is either {@link Ok} or {@link Err},
 * indicating it conforms to the {@link Result} interface.
 *
 * ### Example
 * ```ts
 ...
 *
 * Represents a value that may contain either a successful result ({@link Ok}) or an
 * error ({@link Err}).
 *
 * This class is not intended for direct use by consumers; instead,
 * it serves as the concrete implementation for {@link IResult} interface.
 */
class _Result<T, E> implements IResult<T, E> {
  /**
